...
@section('content')
    ...
    <div id="editModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md">
            <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Edit Paket Soal</h3>
            ...
            <form action="{{ route('questions.update', $editingSet->id) }}" method="POST" class="p-6 sm:p-8 space-y-5">
                ...
                @method('PUT')
                <div>
                    <label for="edit_title" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Judul Paket</label>
                ...
                <div>
                    <label for="edit_key_code" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Kode Unik (Key)</label>
    ...
    {{-- Page Header --}}
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Daftar Soal</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Buat paket soal dengan kode unik, lalu tambahkan pertanyaan ke setiap paket.</p>
    </div>
    ...
    <div class="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div class="w-full lg:w-96 shrink-0">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors sticky top-6">
                <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700">
                    <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Buat Paket Soal Baru</h3>
                </div>
                <form action="{{ route('questions.store') }}" method="POST" class="p-6 space-y-5">
                    @csrf
                    ...
                    {{-- Title --}}
                    <div>
                        <label for="title" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Judul Paket</label>
                        <input type="text" id="title" name="title" value="{{ old('title') }}" placeholder="contoh: UTS Matematika 2026"
                    ...
                    <div>
                        <label for="key_code" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Kode Unik (Key)
                            <span class="ml-1 text-xs font-normal text-gray-400 dark:text-gray-500">(kosongkan untuk otomatis)</span>
                        </label>
                        <input type="text" id="key_code" name="key_code" value="{{ old('key_code') }}" placeholder="contoh: MAT-2026 — atau kosongkan untuk acak"
                    ...
                    <div class="pt-1">
                        <button type="submit"
                                class="w-full inline-flex justify-center items-center gap-2 rounded-md bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 transition-colors">
        ...
        {{-- Right Column: Question Sets List --}}
        <div class="flex-1 w-full min-w-0">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">Paket Soal Anda</h3>
                        <span class="text-xs text-gray-400 dark:text-gray-500">Total {{ $questionSets->total() }}</span>
                    </div>
                    <!-- Search Bar -->
                    <form action="{{ route('questions.index') }}" method="GET" class="relative w-full sm:w-56">
                ...
                <div id="question-list" class="overflow-y-auto" style="max-height: calc(100vh - 14rem);">
                    ...
                    <div class="text-center py-12">
                        ...
                        <p class="text-sm text-gray-400 dark:text-gray-500">Belum ada paket soal. Buat paket pertama Anda di samping.</p>
                    </div>
                    @else
                    <div class="divide-y divide-gray-100 dark:divide-gray-700">
                        @foreach($questionSets as $set)
                        <div class="flex items-center justify-between gap-4 px-5 py-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                ...
                                <a href="{{ route('questions.index', ['edit' => $set->id]) }}"
                                   class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium bg-yellow-50 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 hover:bg-yellow-100 dark:hover:bg-yellow-900/50 transition-colors">
                                ...
                                <form action="{{ route('questions.destroy', $set->id) }}" method="POST" onsubmit="return confirm('Hapus paket soal ini dan semua pertanyaannya?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="p-1.5 rounded-md text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:text-red-400 dark:hover:bg-red-900/30 transition-colors" title="Hapus paket">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
