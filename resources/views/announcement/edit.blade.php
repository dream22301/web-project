...
@section('content')
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4">
            <a href="{{ route('announcement.index') }}" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                </svg>
            </a>
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Pengumuman</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Perbarui detail pengumuman.</p>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-3xl bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors">
        ...
        <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700">
            <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Detail Pengumuman</h3>
        </div>

        <form action="{{ route('announcement.update', $announcement->id) }}" method="POST" class="p-6 sm:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" id="title" name="title" placeholder="contoh: Pemberitahuan Libur Sekolah" value="{{ old('title', $announcement->title) }}"
            ...
            <div>
                ...
                <select id="audience" name="audience"
                    ...
                    <option value="all" {{ old('audience', $announcement->audience) == 'all' ? 'selected' : '' }}>Semua Siswa</option>
                    <option value="x"   {{ old('audience', $announcement->audience) == 'x'   ? 'selected' : '' }}>Kelas X</option>
                    <option value="xi"  {{ old('audience', $announcement->audience) == 'xi'  ? 'selected' : '' }}>Kelas XI</option>
                    <option value="xii" {{ old('audience', $announcement->audience) == 'xii' ? 'selected' : '' }}>Kelas XII</option>
                </select>
                @error('audience')
                    <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
                ...
                <div class="flex flex-wrap gap-4">
                    ...
                    <div class="flex items-center">
                        <input id="priority-{{ $p['value'] }}" name="prioritas" type="radio" value="{{ $p['value'] }}"
                               ...
                               {{ old('prioritas', $announcement->prioritas) == $p['value'] ? 'checked' : '' }}>
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="6"
                          ...
                          class="block w-full rounded-md border-0 py-2.5 px-3 text-gray-900 dark:text-gray-100 bg-gray-50 dark:bg-gray-700 ring-1 ring-inset placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 text-sm transition-colors resize-none
                                 {{ $errors->has('content') ? 'ring-red-400 dark:ring-red-500 focus:ring-red-500' : 'ring-gray-300 dark:ring-gray-600 focus:ring-blue-600' }}">{{ old('content', $announcement->content) }}</textarea>
                @error('content')
                    <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="relative max-w-xs">
                    ...
                    <input type="date" id="publish_date" name="publish_date" value="{{ old('publish_date', $announcement->publish_date) }}"
