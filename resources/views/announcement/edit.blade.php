...
@section('content')
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update announcement details.</p>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-3xl bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors">
        ...
        <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700">
            <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">Announcement Details</h3>
        </div>
        ...
        <form action="{{ route('announcement.update', $announcement->id) }}" method="POST" class="p-6 sm:p-8 space-y-6">
            ...
            <div>
                ...
                <input type="text" id="title" name="title" placeholder="e.g. School Holiday Notice" value="{{ old('title', $announcement->title) }}"
            ...
            <div>
                ...
                <select id="audience" name="audience"
                    ...
                    <option value="all" {{ old('audience', $announcement->audience) == 'all' ? 'selected' : '' }}>All Students</option>
                    <option value="x"   {{ old('audience', $announcement->audience) == 'x'   ? 'selected' : '' }}>Grade X</option>
                    <option value="xi"  {{ old('audience', $announcement->audience) == 'xi'  ? 'selected' : '' }}>Grade XI</option>
                    <option value="xii" {{ old('audience', $announcement->audience) == 'xii' ? 'selected' : '' }}>Grade XII</option>
                </select>
                @error('audience')
                    <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
