    ...
    <div class="flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div class="w-full lg:w-96 shrink-0">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors sticky top-6">
                ...
                <form action="{{ route('student-schedule.store') }}" method="POST" class="p-6 space-y-5">
                    @csrf
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label for="period_start" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Dari Jam</label>
                            <div class="relative">
                                ...
                                <input type="number" id="period_start" name="period_start" value="{{ old('period_start') }}" min="1" max="12" placeholder="1"
                                       class="block w-full pl-10 rounded-md border-0 py-2.5 text-gray-900 dark:text-gray-100 bg-gray-50 dark:bg-gray-700 ring-1 ring-inset placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-600 text-sm transition-colors
                        ...
                        <div>
                            <label for="period_end" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Sampai Jam</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                    <svg class="w-4 h-4 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                ...
                                <input type="number" id="period_end" name="period_end" value="{{ old('period_end') }}" min="1" max="12" placeholder="4"
                                       class="block w-full pl-10 rounded-md border-0 py-2.5 text-gray-900 dark:text-gray-100 bg-gray-50 dark:bg-gray-700 ring-1 ring-inset placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-600 text-sm transition-colors
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">Jadwal Terpublikasi</h3>
                        <span class="text-xs text-gray-400 dark:text-gray-500">{{ $schedules->total() }} aktif</span>
                    </div>
                    <div class="flex items-center gap-3">
                        @if($schedules->total() > 0)
                        <form action="{{ route('student-schedule.clearAll') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus SEMUA jadwal siswa? Tindakan ini tidak dapat dibatalkan.')">
                            @csrf
                            @method('DELETE')
                        ...
                        <!-- Search Bar -->
                        <form action="{{ route('student-schedule.index') }}" method="GET" class="relative w-full sm:w-56">
                ...
                <!-- Schedule Items (internally scrollable) -->
                <div id="schedule_list" class="overflow-y-auto" style="max-height: calc(100vh - 14rem);">
                    @if($schedules->isEmpty())
                        <div class="text-center py-12">
                            ...
                            <p class="text-sm text-gray-400 dark:text-gray-500">Belum ada jadwal siswa. Tambahkan jadwal pertama Anda di samping.</p>
                        </div>
                    @else
                        @php
